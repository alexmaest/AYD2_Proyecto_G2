...
export default function Album (artist: Artist) {
  return (
    <Link href={`/user/artist/${artist?.id}`}>
      <div
      ...
      >
        <div className='flex flex-col items-center relative'>
          ...
          <img src={artist?.linkPhoto} alt={artist?.nombre} className='w-44 h-44 rounded-lg' />
        </div>
        <div className='mx-6'>
          <p className='text-retro-white font-bold py-2 truncate'>{artist?.nombre}</p>
          <p className='text-retro-black-300 text-sm'>{artist?.genero}</p>
        </div>
      </div>
    </Link>
  )
