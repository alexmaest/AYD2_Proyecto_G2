...
const TopGlobalSongs = () => {
  ...
  useEffect(() => {
    const fetchDates = async () => {
      ...
      const url = `http://192.168.0.14:5000/user/historial/${session.id}`
      try {
  ...
  const handleFilter = () => {
    ...
    try {
      ...
      const arrayArtistNames = []
      data?.forEach((item) => {
        if (item.date >= startDate && item.date <= endDate) {
          item?.songs?.forEach((song) => {
            if (!arraySongNames.includes(song.name)) {
              arraySongNames.push(song.name)
            }
          })
        }
      })
      ...
      data?.forEach((item) => {
        if (item.date >= startDate && item.date <= endDate) {
          item?.songs?.forEach((song) => {
            if (!arrayArtistNames.includes(song.artist)) {
              arrayArtistNames.push(song.artist)
            }
          })
        }
      })
      ...
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={styles.Table}>
        ...
        {labels.map((label, index) => (
          <View key={index} style={styles.TableRow}>
            ...
            <Text style={styles.TableCell}>{songsNames[index]}</Text>
          </View>
        ))}
      </ScrollView>
    </SafeAreaView>
